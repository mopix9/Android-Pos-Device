...
import com.fanap.corepos.receipt.sina.SinaSuccessReceipt
import com.fanap.corepos.utils.Utils
import com.fanap.sina.base.BaseFragment
import com.fanap.sina.databinding.FragmentSuccessfulTransactionsBinding
...
@AndroidEntryPoint
class SuccessfulTransactionsFragment  : BaseFragment<FragmentSuccessfulTransactionsBinding>() , TimePickerDialog.OnTimeSetListener, DatePickerDialog.OnDateSetListener{
    ...
    private  var endDate = ""
    private val viewModel : SuccessfulTransactionsViewModel by viewModels()
    ...
    override fun getViewBinding(
        ...
        container: ViewGroup?
    ): FragmentSuccessfulTransactionsBinding = FragmentSuccessfulTransactionsBinding.inflate(inflater,container,false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.refresh.observe(viewLifecycleOwner, { getTransactions() })
    ...
    private fun getMoreItems(offset: Long) = lifecycleScope.launch {
        ...
        val transactions = transactionRepository.getSuccessTransactionsLazy(start, end, viewModel.selectedTransaction.get()!!, offset * 10)
    ...
    private fun getTransactions() = lifecycleScope.launch {
        ...
        if (startDate.isNotBlank() && endDate.isNotBlank()){
            ...
            val transactions = transactionRepository.getSuccessTransactionsLazy(start, end, viewModel.selectedTransaction.get()!!, counter)
            loading.dismiss()
            ...
            isLoading = false
            val print = transactionRepository.getSuccessTransactions(start, end, viewModel.selectedTransaction.get()!!)
            printList.clear()
    ...
    private fun print() = lifecycleScope.launch(Dispatchers.Main) {
                ...
                when {
                    lists.size == 1 -> {
                        withContext(Dispatchers.Main) {
                            bitmap = SinaSuccessReceipt().generateReceipt(requireContext(), list,printList, PrintPart.All, headerData)
                        }
                        bitmap?.let {
                    ...
                    index == 0 -> {
                        withContext(Dispatchers.Main) {
                            bitmap = SinaSuccessReceipt().generateReceipt(requireContext(), list,printList, PrintPart.Header, headerData)
                        }
                        bitmap?.let {
                            result = printer?.printBitmap(it) ?: false
                        }
                    ...
                    index == lists.size - 1 -> {
                        withContext(Dispatchers.Main) {
                            bitmap = SinaSuccessReceipt().generateReceipt(requireContext(), list,printList, PrintPart.Footer, headerData)
                        }
                        bitmap?.let {
                            result = printer?.printBitmap(it) ?: false
                        }
                        ...
                        withContext(Dispatchers.Main) {
                            bitmap = SinaSuccessReceipt().generateReceipt(requireContext(), list,printList, PrintPart.Body,headerData)
                        }
                        bitmap?.let {
                            result = printer?.printBitmap(it) ?: false
                        }
        ...
        loading.dismiss()
        if (result)
            finish(this@SuccessfulTransactionsFragment)
        else

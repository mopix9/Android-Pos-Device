...
import com.masa.aryan.balance.viewmodel.BalanceViewModel
import com.masa.aryan.base.BaseFragment
import com.masa.aryan.databinding.FragmentBalanceBinding
...
class BalanceFragment : BaseFragment<FragmentBalanceBinding>() {
   ...
   /* val mToastDuration =90000000
    fun mDisplayToast(toast: Toast){
        Thread{
            for(i in 1..mToastDuration){
                toast.show()
...
//        Utils.makeSnack(binding.root,"کارمزد 1206 ریال",Snackbar.LENGTH_LONG).show()
            ...
            withContext(Dispatchers.Main) {
                if (result != null) {
                    ...
                    if (responseCode == "00") {
                        ...
                        viewModel.updateTransaction(transaction)
                        navigate(this@BalanceFragment, R.id.action_balanceFragment_to_failFragment,

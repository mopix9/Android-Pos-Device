package com.fanap.corepos.device.hsm.sina.amp;
...
public class AmpHSMSina implements HSMInterface, AmpPinActivity.PinResult {
    ...
    private int macLeftKeyId = 5;
    private int macRightKeyId = 6;

    private String masterKey;
    String TAG = "AmpHSMSina";
    private static AmpHSMSina ampHSM;
    ...
    public boolean loadMacKey(String macKey) {
        byte[] IV = new byte[8];
        byte[] plainMacKey = pinpadProvider.desDencryptUnified(KeySystem.FIXED_DES, KeyType.KEY_TYPE_FIXEAK, mastKeyIndex2, Ped.TDEA_DECRYPT, IV, IsoUtil.hexStringToByteArray(macKey));
        byte[] leftKey =  Arrays.copyOfRange(plainMacKey, 0, 8);
        byte[] rightKey = Arrays.copyOfRange(plainMacKey, 8, 16);
        ...
        result = loadLeftKey(leftKey);
        result = loadRightKey(rightKey);
        if (result) {
    ...
    private boolean loadLeftKey(byte[] left) {
        nRet = pinpadProvider.injectKey(KeySystem.FIXED_DES, KeyType.KEY_TYPE_FIXEAK, macLeftKeyId, left);
        if (nRet == 0) {
    ...
    private boolean loadRightKey(byte[] right) {
        nRet = pinpadProvider.injectKey(KeySystem.FIXED_DES, KeyType.KEY_TYPE_FIXEAK, macRightKeyId, right);
        if (nRet == 0) {
            Log.d(TAG, "Write DES key ok");
            return true;
        } else {
    ...
    @Override
    public boolean loadDataKey(String desKey) {
        byte[] IV = new byte[8];
        byte[] plainDataKey = pinpadProvider.desDencryptUnified(KeySystem.FIXED_DES, KeyType.KEY_TYPE_FIXEAK, mastKeyIndex2, Ped.TDEA_DECRYPT, IV, HSMUtils.hexStringToByte(desKey));
        nRet = pinpadProvider.injectKey(KeySystem.FIXED_DES, KeyType.KEY_TYPE_FIXEAK, desKeyId, plainDataKey);
        if (nRet == 0) {
            Log.d(TAG, "Write DES key ok");
            return true;
        } else {
    ...
    public byte[] calcMac(byte[] macData) {
        macData = convertToEightMod(macData);
        byte[] result = new byte[8];
        byte[] datablock = new byte[8];
        int remain = macData.length % 8;
        int LoopCount = macData.length / 8;
        ...
        System.arraycopy(macData,0,datablock, 0, 8);
        result = encryptWithLeftMacKey(datablock);

        for (int i = 1; i < LoopCount; i++)
        {
            datablock = new byte[8];
            System.arraycopy(macData, i * 8, datablock, 0, 8);
            datablock = IsoUtil.xorArray(datablock, result);
            result = encryptWithLeftMacKey(datablock);
        }

        byte[] LastBlock = new byte[8];

        System.arraycopy(macData, macData.length - remain, LastBlock, 0, remain);
        LastBlock = IsoUtil.xorArray(LastBlock, result);

        result = encryptWithLeftMacKey(LastBlock);

        result = decryptWithRightMacKey(result);

        result = encryptWithLeftMacKey(result);
    ...
    public String decryptionData(String encryptedData) {
      ...
      //  byte[] data = {(byte)0x55,(byte)0x6C,(byte)0xE9,(byte)0x07,(byte)0x86,(byte)0xAF,(byte)0x37,(byte)0x8B,(byte)0x2A,(byte)0xE2,(byte)0xCF,(byte)0x97,(byte)0x15,(byte)0xEB,(byte)0x73,(byte)0xE7};
        byte[] retData = pinpadProvider.desDencryptUnified(KeySystem.FIXED_DES, KeyType.KEY_TYPE_FIXEAK, desKeyId, Ped.TDEA_DECRYPT | Ped.TDEA_MODE_ECB, IV, HSMUtils.hexStringToByte(encryptedData));
    ...
    private byte[] decryptWithRightMacKey(byte[] encryptedData) {
        byte[] IV = new byte[8];
        byte[] retData = pinpadProvider.desDencryptUnified(KeySystem.FIXED_DES, KeyType.KEY_TYPE_FIXEAK, macRightKeyId, Ped.TDEA_DECRYPT | Ped.TDEA_MODE_ECB, IV, encryptedData);

        if (retData != null) {
            Log.d(TAG, "Data Decrypted： " + BCDHelper.bcdToString(retData, 0, retData.length));
            return retData;
        } else {
            Log.d(TAG, "Decrypte fail");
            return null;
        }
    }

    private byte[] encryptWithLeftMacKey(byte[] plainData) {
        byte[] IV = new byte[8];
        byte[] retData = pinpadProvider.desDencryptUnified(KeySystem.FIXED_DES, KeyType.KEY_TYPE_FIXEAK, macLeftKeyId, Ped.TDEA_ENCRYPT | Ped.TDEA_MODE_ECB, IV, plainData);
    ...
    public String encryptionData(String data) {
        /*try {
            byte[] reData = pinpadProvider.calcDes(desKeyId, PinpadConstant.TDES_TYPE_ENCRYPT, MyApplication.utils.ascToBcd(data));
            if (reData != null) {

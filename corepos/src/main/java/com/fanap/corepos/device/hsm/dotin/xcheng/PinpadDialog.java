...
import com.fanap.corepos.device.hsm.aryan.smarpeak.pinpad.OnPinpadDialogListener;
...
public class PinpadDialog {
	...
	private int keyMode = PosSecurityManager.PED_PINBLOCK_FETCH_MODE_TPK;

	private String  pinCard;
	private int     pinType;
	private boolean pinBypass;
	private PosSecurityManager     hsmManage;
	...
	public PinpadDialog(Context context, String s, int area, int index, int i, int i1, OnPinpadDialogListener listener) {
	...
	public PinpadDialog(Context context,  Bundle bundle, int pinKeyIndex) {
		...
		this.hsmManage = PosSecurityManager.getDefault();
...
/*	if (bundle.containsKey(EmvPinConstraints.PIN_CARD)) {
	...
	private class PinEventListener implements PosSecurityManager.EventListener {
		...
		@Override
		public void onPedVerifyPin(PosSecurityManager manage, int type, byte[] rspBuf) {
			if (type == PosSecurityManager.PED_VERIFY_PIN_TYPE_PLAIN || type == PosSecurityManager.PED_VERIFY_PIN_TYPE_CIPHER) {
		...
		@Override
		public void onPedPinBlockRet(PosSecurityManager manage, int type, byte[] rspBuf) {
			if (rspBuf[0] != 0) {
		...
		@Override
		public void onKeyboardShow(PosSecurityManager manage, byte[] keys, int timeout) {

		}

		@Override
		public void onKeyboardInput(PosSecurityManager manage, int numKeys) {
			String info = "";
			while (0 != (numKeys--))
				info += "*";
			if (info.length() <= 12)
		...
		@Override
		public void onInfo(PosSecurityManager manage, int what, int extra) {
			Log.e(TAG, "onInfo");
		}

		@Override
		public void onError(PosSecurityManager manage, int what, final int extra) {
		...
		@Override
		public void onHwSelfCheckRet(PosSecurityManager manage, int type, int checkResult) {
			Log.e(TAG, "onHwSelfCheckRet");
		}

		@Override
		public void onHwSensorTriggered(PosSecurityManager manage, int triggered, byte[] sensorValue, byte[] triggerTime) {
			Log.e(TAG, "onHwSensorTriggered");
		}

		@Override
		public void onPedKeyManageRet(PosSecurityManager manage, int ret) {

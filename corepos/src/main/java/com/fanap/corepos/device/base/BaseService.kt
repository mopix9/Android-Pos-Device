package com.fanap.corepos.device.base
...
object BaseService {
 ...
 fun ShowPermissions(): Array<String>? {
  val packManager: PackageManager =
   com.fanap.corepos.device.base.BaseService.androidContext!!.getPackageManager()
  val infos = packManager.getInstalledApplications(PackageManager.GET_ACTIVITIES)
  var info: ApplicationInfo? = null
  try {
   info = packManager.getApplicationInfo(
    com.fanap.corepos.device.base.BaseService.androidContext!!.getPackageName(),
 ...
 fun validatePermission(permission: String?) {
  ...
  try {
   // LOG.D("检查" + permission);
   val permiStrings: Array<String>? =com.fanap.corepos.device.base.BaseService.ShowPermissions()
   valid = permiStrings?.let { permission?.let { it1 ->
    com.fanap.corepos.device.base.BaseService.ifInside(it,
     it1
    )
   } } == true

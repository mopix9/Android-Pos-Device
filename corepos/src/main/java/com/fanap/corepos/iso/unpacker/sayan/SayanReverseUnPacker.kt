...
import com.fanap.corepos.iso.unpacker.sayan.schema.SayanAdviseReverseUnPackSchema
import com.fanap.corepos.iso.utils.IsoFields
import com.fanap.corepos.utils.IsoUtil
import java.util.HashMap

class SayanReverseUnPacker : UnPacker() {
    ...
    override fun unpack(): HashMap<IsoFields, String> {
        ...
        for (i in bitmap.indices)
            if (bitmap[i] == 1) {
                val schema: SinaIsoField = SayanAdviseReverseUnPackSchema.getIsoFieldInfo(i + 1)
                var fieldLength: Int
                when (schema.type) {

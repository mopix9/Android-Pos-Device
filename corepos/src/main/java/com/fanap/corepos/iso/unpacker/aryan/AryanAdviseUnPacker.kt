...
import com.fanap.corepos.iso.model.AryanDataTypes
import com.fanap.corepos.iso.model.AryanFieldTypes
import com.fanap.corepos.iso.model.AryanIsoField
import com.fanap.corepos.iso.model.IsoFieldLengthType
import com.fanap.corepos.iso.unpacker.aryan.schema.AryanAdviseReverseUnPackSchema
import com.fanap.corepos.iso.unpacker.base.UnPacker
import com.fanap.corepos.iso.utils.IsoFields
import com.fanap.corepos.utils.IsoUtil
import java.util.*

class AryanAdviseUnPacker : UnPacker() {
    ...
    override fun unpack(): HashMap<IsoFields, String> {
        ...
        for (i in bitmap.indices)
            if (bitmap[i] == 1) {
                val schema: AryanIsoField = AryanAdviseReverseUnPackSchema.getIsoFieldInfo(i + 1)
                var fieldLength: Int
                when (schema.type) {
                    AryanFieldTypes.N, AryanFieldTypes.AN, AryanFieldTypes.ANS -> {
                        when (schema.lengthType) {
                            IsoFieldLengthType.LLL -> {
                                fieldLength =
                                    message.sliceArray(pointer until pointer + 4).concatToString()
                                        .toInt() * 2
                                pointer += 4
                                msg[getFieldName(i + 1)] =
                                    message.sliceArray(pointer until pointer + fieldLength)
                                        .concatToString()
                                pointer += fieldLength
                            }
                            IsoFieldLengthType.CONST -> {
                                if (schema.dataType == AryanDataTypes.ASCII) {
                                    ...
                                    msg[getFieldName(i + 1)] = IsoUtil.hexToAscii(
                                        message.sliceArray(pointer until pointer + fieldLength)
                                            .concatToString()
                                    )
                                    pointer += fieldLength
                                    ...
                                    msg[getFieldName(i + 1)] =
                                        message.sliceArray(pointer until pointer + schema.length)
                                            .concatToString()
                    ...
                    AryanFieldTypes.B -> {
                        fieldLength = schema.length * 2
                        msg[getFieldName(i + 1)] =
                            message.sliceArray(pointer until pointer + fieldLength).concatToString()
                        pointer += fieldLength
                    }
                    else -> {}
                }
    ...
    private fun getFieldName(field: Int): IsoFields {
        return when (field) {
            ...
            15 -> IsoFields.SettlementDate
            24 -> IsoFields.NiiCode
            25 -> IsoFields.ConditionCode
            37 -> IsoFields.Rrn

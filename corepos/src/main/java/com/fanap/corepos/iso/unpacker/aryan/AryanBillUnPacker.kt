...
class AryanBillUnPacker  : UnPacker() {

    override fun unpack(): HashMap<IsoFields, String> {

        val temp: CharArray = IsoUtil.bytesToHex(message).toCharArray()
        val message: CharArray
        if (temp.sliceArray(0..3).concatToString() == "0000")
            message = temp.sliceArray(4 until temp.size)
        else
            message = temp
        ...
        msg[IsoFields.BankName] = Encoding.convertToWindows1256(IsoUtil.hexStringToByteArray(getFieldWithTagName(msg[IsoFields.Buffer1],"38")))
        msg[IsoFields.BillName] = Encoding.convertToWindows1256(IsoUtil.hexStringToByteArray(getFieldWithTagName(msg[IsoFields.Buffer1],"51")))
